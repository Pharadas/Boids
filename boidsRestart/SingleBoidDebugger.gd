...
#		set_applied_force(resultantBoidPush.normalized() * 200 * maxForce * 2)
#		set_applied_force(currAveHeading.normalized() * 200 * maxForce * 2)
#		set_applied_force((currCenter / boidsFound).normalized() * maxForce * 2)
#		set_applied_force(Vector2(global_position.x - currCenterX, global_position.y - currCenterY).normalized() * boidsFound * 200)
#		$Line2D.centerOfBoids(get_global_position(), Vector2(currCenterX, currCenterY))
#	else:
#		$Line2D.cleanAllLines()
